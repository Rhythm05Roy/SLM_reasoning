"""training/grpo_trainer.py — Three-stage curriculum GRPO."""
...
def _run_stage(stage: int, tool_ratio: float, steps: int, config,
               model, tokenizer, grpo_dataset: Dataset,
               curriculum_callback, reward_fns: List) -> dict:
    from trl import GRPOConfig, GRPOTrainer

    logger.info(f"\n{'='*50}\nGRPO Stage {stage}  tool_ratio={tool_ratio}  steps={steps}\n{'='*50}")

    stage_data = _stage_dataset(
        grpo_dataset.select(range(min(len(grpo_dataset), 1200))),
        tool_ratio,
    )

    grpo_cfg = GRPOConfig(
        output_dir=str(Path(config.output_dir) / f"grpo_stage_{stage}"),
        learning_rate=config.grpo_lr * (0.5 ** stage),
        adam_beta1=0.9,
        adam_beta2=0.99,
        ...
        optim="adamw_8bit",                       # Fix: was paged_adamw_8bit
        ...
        run_name=(f"{config.wandb_run_name}_grpo_s{stage}" if config.use_wandb else None),
    ...
    logger.info(f"Stage {stage} saved → {stage_ckpt}  loss={result.training_loss:.4f}")
    torch.cuda.empty_cache()

    return {"stage": stage, "tool_ratio": tool_ratio,
            "loss": result.training_loss, "steps": steps}


def run_grpo(config, model, tokenizer, grpo_dataset: Dataset, curriculum) -> List[dict]:
    ...
    for stage, tool_ratio, include_intern in stage_configs:
        if stage < start_stage:
            logger.info(f"Skipping stage {stage} (resuming from {start_stage})")
            continue

        cb = CurriculumCallback(curriculum)
        reward_fns = get_reward_functions(include_internalization=include_intern)
        metrics = _run_stage(
            stage=stage, tool_ratio=tool_ratio, steps=steps_per_stage,
            config=config, model=model, tokenizer=tokenizer,
            grpo_dataset=grpo_dataset, curriculum_callback=cb,
    ...
    for m in stage_metrics:
        logger.info(f"  Stage {m['stage']}  tool_ratio={m['tool_ratio']}  loss={m['loss']:.4f}")
    logger.info(f"Curriculum: {curriculum.get_state()}")
